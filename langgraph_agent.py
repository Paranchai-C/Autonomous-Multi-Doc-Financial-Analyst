...
def retrieve_node(state: AgentState):
    ...
    router_prompt = f"""
    Analyze the user question and classify the user's question into one of four categories: ["apple", "tesla", "both", "none"].
...
def rewrite_node(state: AgentState): 
    ...
    msg = [ 
        HumanMessage(content=f"The previous search for '{question}' yielded irrelevant results.\n"
                             f"Please rewrite the original question to be more specific or use better financial terminology.\n"
                             f"You must transform vague queries (e.g., 'how much did they spend on new tech') into precise terms (e.g., 'Research and Development expenses').\n"
...
def run_legacy_agent(question: str):
    ...
    template = """Answer the following questions as best you can. You have access to the following tools:
                ...
                1. English Only: The Final Answer must be in English, even if the user asks in Chinese or another language.
                2. Year Precision: You must strictly distinguish between 2024, 2023, and 2022 columns in financial tables. Do not mix them up.
                3. Honesty: If the exact figure is not found in the documents, you must output exactly "I don't know" rather than guessing.
